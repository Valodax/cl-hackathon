...
describe(`Static Raffle test scenario with seven winners`, async function () {
    async function deployStaticRaffleFixture() {
        ...
        const mockVrfCoordinator: VRFCoordinatorV2Mock = await vrfCoordinatorFactory.deploy(
            BASE_FEE,
            GAS_PRICE_LINK
        );
        ...
        const subscriptionId = ethers.BigNumber.from(txReceipt.events[0].topics[1]); // returns a string but we convert to big number

        // grab from the doccs (use the one for Goerli testnet)
    ...
    describe(`Running raffle scenario`, async function () {
        it(`should run raffle and determine seven winners only once`, async function () {
            ...
            assert(winners.length === fixture.numWords, "Invalid number of winners");
        });
    });
});
